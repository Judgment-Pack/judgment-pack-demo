...
- **Visualize**: call the **`get_pack_diagram` MCP tool** — its output is the pack's
  ...
  **[Open the zoomable viewer](http://localhost:8002/enterprise-demo/diagrams/<decision-id>.html)** — scroll to zoom, drag to pan.
  ...
  The fence is what makes GitHub render it as a diagram; a file without it shows as plain
  text. Then run `sh scripts/render-diagram.sh` from the repo root and tell the user:
  **the diagram is in the viewer at
  http://localhost:8002/enterprise-demo/diagrams/<decision-id>.html — start on Core flow.**
  Never mention VS Code; the viewer is the picture. Node meanings: member nodes quote the pack; `unresolved (unknown)` /
